...
async function update(id, params) {
  ...
  if (params.email && account.email !== params.email && (await getAccount({ email: params.email }))) {
...
function basicDetails(account) {
  const { id, firstName, lastName, email, role, created, updated, isVerified, image_url, phoneNumber } = account;
  return {
    id,
    firstName,
    lastName,
