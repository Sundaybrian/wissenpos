...
describe('GET /api/v1/company/:company_id/order/:id', () => {
  ...
  it('should fail to find a cart', async () => {
    const res = await request(app).get('/api/v1/company/1/order/1').expect(404);

    // expect(res.body.message).toContain("Validation error");
  });

  it('shoul retun cart and items', async () => {
    const res = await request(app).get('/api/v1/company/1/order/xtreeme1').expect('Content-Type', /json/).expect(200);
...
describe('GET /api/v1/company/:company_id/order/my-orders', () => {
  ...
  it('should fail to find orders if cart id is not present', async () => {
    const res = await request(app).get('/api/v1/company/1/order/my-orders').expect(500);

    expect(res.body.message).toContain(`Validation error`);
  });
  it('should return 404 for my orders', async () => {
    const res = await request(app).get('/api/v1/company/1/order/my-orders').expect(404);
  });
