...
async function updateCartItem(id, params) {
  ...
  if (params.item.quantity <= 0) {
    // remove from cart
    orderItem = await order.$relatedQuery('items').delete().where({ item_id: params.product_id });
...
async function basicDetails(order) {
  const { cart_id, order_status, purchase_status, subtotal, company_id, items } = order;
